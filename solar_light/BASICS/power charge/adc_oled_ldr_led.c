...
const int LED = 5;                              // Relay: LED_Strip
const int buttonPin = 11;                       // Pushbutton pin
const int ledPin = 13;                          // BUilt-in LED pin

int sensorValue;
float voltage;
float Volt = 0.00;                              // printed voltage
...
void setup() {
  Serial.begin(9600);
  pinMode(LED, OUTPUT);                        // Relay: LED strip
...
void loop() {
  ...
  if(Volt <= 4.00){
    state = !state;                               // Toggle state (LOW to HIGH, or HIGH to LOW)
    digitalWrite(LED, state);                     // Write to pin
  }
}
...
void testdrawstyles(void) {
  ...
  display.setTextColor(SSD1306_WHITE);
   ...
   display.print(Volt); display.println(F("V"));
