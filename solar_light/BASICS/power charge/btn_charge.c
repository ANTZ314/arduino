...
const int Vchrg = 4;                            // Relay: V-charge
const int Vsol  = 2;                            // Relay: V-charge
...
bool state = LOW;                               // Track state


/* Startup Routine */
void setup() {
  Serial.begin(9600);
  pinMode(Vchrg, OUTPUT);                       // Relay: Charge
...
void loop() {
  ...
  if (buttonState == HIGH) {
    ...
    digitalWrite(ledPin, state);                // turn LED on
    Serial.println("PRESSED");                  // REMOVE
    delay(500);                                 // Debounce press
    /* Read volatge and display */
    digitalWrite(Vsol, state);                  // Close Solar-Input Relay
    delay(100);
    digitalWrite(Vchrg, state);                 // Close Charge Relay
    adc_read();                                 // Read Battery voltage
    testdrawstyles();                           // Display on OLED
  } else {
    digitalWrite(ledPin, LOW);                  // turn LED off
    //digitalWrite(Vchrg, LOW);                 // Open Charge Relay - Toggling
  }
}
