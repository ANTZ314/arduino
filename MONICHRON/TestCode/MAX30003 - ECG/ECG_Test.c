/*
 * Adapted from: PROTOCENTRAL MAX30003 SINGLE-LEAD ECG
 * Changed Pin numbers from Arduino UNO to ESP32 
 * 
 * SDI = MOSI - VSPID   - IO_23 (data_out)
 * SDO = MISO - VSPIQ   - IO_19 (data_in)
 * SCL = SCLK - VSPICLK - IO_18 (clock)
 * CS  = CS   - VSPICS0 - IO_05 (chip select)
 * ECG_FCLK				- IO_25	(output 32.768 kHz)
 * 
 */
#include<SPI.h>
#include <TimerOne.h>
...
#define MAX30003_CS_PIN  5		//7
#define CLK_PIN          25		//6		-> FCLK?
...
void MAX30003_begin()
{
	...
	//(or 0.1 sec - or 10Hz => 5 On & 5 Off per second)
